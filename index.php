...
$host = '178.159.44.173:3306';
$database = 'by_car_parser'; 
$user = 'manager'; 
$password = 'cei5iQui'; 

// выполняем подключение к серверу
$connection = mysql_connect($host, $user, $password) 
or die("Ошибка подключения " . mysql_error($connection));

// выбираем бд
mysql_select_db($database) or die ("Ошибка выбора бд " . mysql_error());

// выполняем различные операции с базой данных

// получение уникальных объявлений
$query = "SELECT count(distinct(id)) FROM ads where status='uniq';";
$unique_ads = mysql_query($query) or die(mysql_error());

// получение пользователей сегодня
$query = "SELECT count(distinct(id)) FROM by_car_parser.aauth_users where banned='0'";
$users_today = mysql_query($query) or die(mysql_error());

// получение уведомлений за сегодня
$query = "SELECT SUM(T1.cnt)
FROM
(
SELECT COUNT(id) AS cnt FROM viber_free_queue where is_new='0' and date(created)=current_date()
UNION
SELECT COUNT(id) AS cnt FROM viber_queue where is_new='0' and date(created)=current_date()
)T1";
$notifications_today = mysql_query($query) or die(mysql_error());
...
$draw->setFont(BASEPATH."/font/".$numbers_font);
$draw->setTextAlignment(Imagick::ALIGN_CENTER);
$draw->setFillColor("rgb(".$numbers_font_color.")");
$draw->setFontSize($today_font_size);

// Рисование уникальных объявлений за сегодня
$unique_ads_x = 826;
$unique_ads_y = 335;
$bg->annotateImage($draw, $unique_ads_x, $unique_ads_y, 0, mb_strtoupper((integer)4322));

// Рисование пользователей сегодня
$users_today_x = 592;
$users_today_y = 335;
$bg->annotateImage($draw, $users_today_x, $users_today_y, 0, mb_strtoupper((integer)1704));

// Рисование уведомдлений за сегодня
$notifications_today_x = 1068;
$notifications_today_y = 335;
$bg->annotateImage($draw, $notifications_today_x, $notifications_today_y, 0, mb_strtoupper((integer)7845));
